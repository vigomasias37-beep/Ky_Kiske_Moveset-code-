...
unsafe extern "C" fn ken_hadoken_game_regular(agent: &mut L2CAgentBase) {
...
// Pre
unsafe extern "C" fn ken_hadoken_start_pre(weapon: &mut L2CWeaponCommon) -> L2CValue {
...
// Main
unsafe extern "C" fn ken_hadoken_start_main(weapon: &mut L2CWeaponCommon) -> L2CValue {
    MotionModule::change_motion(weapon.module_accessor, Hash40::new("regular"), 0.0, 1.0, false, 0.0, false, false);
    weapon.fastshift(L2CValue::Ptr(ken_hadoken_start_main_loop as *const () as _))
}

unsafe extern "C" fn ken_hadoken_start_main_loop(weapon: &mut L2CWeaponCommon) -> L2CValue {
    // Declare owner boma
...
pub fn install() {
    Agent::new("ken_hadoken")
        .game_acmd("game_regular", ken_hadoken_game_regular, Default)
        .status(Pre, * WEAPON_RYU_HADOKEN_STATUS_KIND_MOVE, ken_hadoken_start_pre)
        .status(Main, * WEAPON_RYU_HADOKEN_STATUS_KIND_MOVE, ken_hadoken_start_main)
        .install();
}
